...
import torch
from transformers import AutoTokenizer, LlavaNextVideoForConditionalGeneration
...
def generate_embeddings_with_llavanv(
    description_tools_mapping,
    model_path="/home/storage/wenbinxing/checkpoints/LLaVA-NeXT-Video-7B",
    output_path="tools_embeddings_llavanv.pkl",
    batch_size=8,
    max_length=512,
    save_dtype="float32",
):
...
def parse_args():
    parser = argparse.ArgumentParser(description="Generate tool embeddings with LLaVA-NeXT-Video language backbone.")
    parser.add_argument(
        "--tools_json",
        type=str,
    ...
    parser.add_argument(
        "--output_path",
        type=str,
        default=str(Path(__file__).resolve().parent / "tools_embeddings_llavanv.pkl"),
    )
...
def main():
    ...
    generate_embeddings_with_llavanv(
