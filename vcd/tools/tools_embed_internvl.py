...
def generate_embeddings_with_internvl(
    description_tools_mapping,
    model_path="../../checkpoints/InternVL3_5-8B",
    output_path="tools_embeddings_internvl.pkl",
    batch_size=8,
    max_length=512,
    save_dtype="float32",
):
...
def parse_args():
    parser = argparse.ArgumentParser(description="Generate tool embeddings with InternVL language backbone.")
    parser.add_argument(
        "--tools_json",
        type=str,
    ...
    parser.add_argument(
        "--model_path",
        type=str,
        default="/home/storage/wenbinxing/checkpoints/InternVL3_5-8B",
    )
    parser.add_argument(
        "--output_path",
        type=str,
        default=str(Path(__file__).resolve().parent / "tools_embeddings_internvl.pkl"),
    )
...
def main():
    ...
    generate_embeddings_with_internvl(
