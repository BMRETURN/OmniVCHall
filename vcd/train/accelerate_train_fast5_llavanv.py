...
from vcd_new.utils_llavanv import (
    PatchProcessor,
...
def configure_llavanv_runtime(args):
    LLAVANV_RUNTIME_CFG["max_num_frames"] = max(1, int(args.llava_num_frames))
...
def infer_single_item(
    ...
    orig_logits, last_hidden_states, _ = answer_question_original(
        ...
        video_frames=frames,
        max_num_frames=LLAVANV_RUNTIME_CFG["max_num_frames"],
    )
...
def build_parser():
    parser = argparse.ArgumentParser(description="Fast distributed training entry (5-GPU) for vcd_new + LLaVA-NeXT-Video")
    ...
    parser.add_argument(
        "--model_dir",
        type=str,
        default="/home/storage/wenbinxing/checkpoints/LLaVA-NeXT-Video-7B",
    )
    parser.add_argument(
        "--dataset_root",
        type=str,
    ...
    parser.add_argument(
        "--tools_embeddings",
        type=str,
        default=str(Path(__file__).resolve().parents[1] / "tools" / "tools_embeddings_llavanv.pkl"),
    )
    parser.add_argument(
        "--dino_dir",
        type=str,
    ...
    parser.add_argument("--llava_num_frames", type=int, default=32)
    return parser


def main():
    args = build_parser().parse_args()
    configure_llavanv_runtime(args)
    ...
    log(f"Loading LLaVA-NeXT-Video model and processor on rank={rank}, device={device} ...", rank)
