...
from vcd_new.tools.positive_tools_vchatf import DINOv3SaliencyExtractor, MotionSaliencyExtractor
from vcd_new.train.optimizer import VCDPolicy, VCDTrainer
from vcd_new.utils_vchatf import (
    PatchProcessor,
...
def configure_vchatf_runtime(args):
    VCHATF_RUNTIME_CFG["max_num_frames"] = max(1, int(args.vchat_max_num_frames))
...
def infer_single_item(
    ...
    orig_logits, last_hidden_states, _ = answer_question_original(
        ...
        video_frames=frames,
        max_num_frames=VCHATF_RUNTIME_CFG["max_num_frames"],
    )
...
def build_parser():
    parser = argparse.ArgumentParser(description="Fast distributed training entry (5-GPU) for vcd_new + VideoChat-Flash")
    ...
    parser.add_argument(
        "--tools_embeddings",
        type=str,
        default=str(Path(__file__).resolve().parents[1] / "tools" / "tools_embeddings_vchatf.pkl"),
    )
    parser.add_argument(
        "--dino_dir",
        type=str,
    ...
    parser.add_argument("--vchat_max_num_frames", type=int, default=64)
    return parser


def main():
    args = build_parser().parse_args()
    configure_vchatf_runtime(args)
    ...
    log(f"Loading VideoChat-Flash model and tokenizer on rank={rank}, device={device} ...", rank)
