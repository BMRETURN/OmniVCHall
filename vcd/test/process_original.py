...
def simple_filter_json(json_file, test_videos_file, output_file=None):
    """
    简化版：根据test_videos.txt过滤JSON文件
    """
    # 读取test_videos.txt
    ...
    # 过滤数据
    filtered_data = [item for item in data if item.get('video_id') in valid_ids]
    ...
    print(f"原始数据: {len(data)}, 过滤后: {len(filtered_data)}")
    return filtered_data


if __name__ == "__main__":
    # 使用示例
    directory = "/home/storage/wenbinxing/Mybenchmark/vcd/test/video_generated/Qwen3-VL-4B-Instruct"
    files = ["m_mcqa_Qwen3-VL-4B-Instruct.json", "m_ynqa_Qwen3-VL-4B-Instruct.json", 
            "s_mcqa_Qwen3-VL-4B-Instruct.json", "s_ynqa_Qwen3-VL-4B-Instruct.json"]

    for file in files:
        input_path = f"{directory}/{file}"
        if os.path.exists(input_path):
            result = simple_filter_json(input_path, "test_videos.txt")
            print(f"已处理 {file}")
...
#     qa_type = "s_ynqa"
#     predictions_dir = "video_generated/Qwen3-VL-8B-Instruct"
#     ground_truth_dir = "video_generated/GT"
